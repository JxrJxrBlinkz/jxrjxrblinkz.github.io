...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		buildlist = pyxbmct.List(buttonFocusTexture=os.path.join(ART , '%s.png' %  HIGHLIGHT_LIST2))
...
window.placeControl(MaintButton, 2,11, 9, 8)
...
window.placeControl(BackResButton, 2, 21, 9, 8)
...
window.placeControl(ToolsButton,2 , 31, 9, 8)
...
ToolsButton.controlRight(CloseButton)  # InstallablesButton if ever fixed
...
CloseButton.controlLeft(ToolsButton)  # InstallablesButton if ever fixed
